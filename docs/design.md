...
- `WithDeepCopy()` deep-copies the callback input subtree and enables `Context.Original` / `Context.TypeOf` resolution for deep-copied descendants.
- If deep copy preparation fails, that mutant is emitted as immediate `Errored` (no shallow-copy fallback).
...
1. Prepare callback input node.
   - Default: shallow-copy target node with `astcow.ShallowCopyNode`.
   - `WithDeepCopy()`: deep-copy target subtree with `astcow.DeepCopyNode`.
   - `internal/astcow/copy_replace.go` is generated by `go generate ./internal/astcow`
     via `internal/astcow/cmd/astcowgen` and should not be edited manually.
2. Build callback context (`Fset`, package, file, types info, path) and register clone-origin mapping.
   - Default: root callback node mapping only.
   - `WithDeepCopy()`: full deep-copied subtree mapping.
